...
public class TeacherControllerImpl implements TeacherControllerInter {
    ...
    @Override
    @GetMapping("/delete")
    public BackReturn delete(@RequestParam(value = "id") Integer teacherId,@RequestParam(value = "cardId")  String IdCardNumber) {
        return teacherService.delete(teacherId,IdCardNumber);
    }
    ...
    @Override
    @GetMapping("/select")
    public BackReturn select(@RequestParam(value = "name",defaultValue = "",required = false) String teachername,@RequestParam(value = "branch",defaultValue = "",required = false) Integer branch) {
